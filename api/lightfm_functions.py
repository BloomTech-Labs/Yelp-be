...
def get_lightfm_model():
    ...
    S3.download_file('yelpsense', 'models/recommender/lightfm/lightfm_model.p', './lightfm/lightfm_model.p')
...
def get_lightfm_dataset():
    ...
    S3.download_file('yelpsense', 'models/recommender/lightfm/lightfm_empty_dataset.p', './lightfm/lightfm_empty_dataset.p')

    empty_dataset = pickle.load( open( "./lightfm/lightfm_empty_dataset.p", "rb" ) )
...
def make_lightfm_user_set(dataset, businesses, stars):
    ...
    userframe['recommend'] = userframe['stars'].map({1:-1, 2:-1, 3:-1, 4:1, 5:1})
...
def lightfm_inference(model, user_interactions, user_dataset):
    ...
    businessframe['lightFM_mapping'] = businessframe['business_id'].apply(lambda x: user_dataset._item_id_mapping[x])
    ...
    top_ten = businessframe[['business_id', 'name', 'address', 'city', 'aggregate_rating', 'categories', 'recommender_values']].sort_values(by='recommender_values',ascending=False)
